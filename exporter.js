...
const fetchContentfulAssets = () => {
    ...
    return Promise.allSettled([
        ...
        contentful
            .getEntries({
                content_type: 'benachrichtigung',
                include: 2
            })
            .then(result => {
                const lastItem = result.items[result.items.length - 1]
                const latest = lastItem ? {
                    title: lastItem.fields['title'],
                    content: lastItem.fields['content'],
                    media: {
                        title: lastItem.fields['media'].fields['title'],
                        url: 'https:' + lastItem.fields['media'].fields['file'].url
                    }
                } : null
                fs.writeFileSync(pathPrefix + 'notification.json', JSON.stringify({latest}, null, 2), 'utf-8')
            }),
...
fetchContentfulAssets()
    .then(() => console.log('Fetching Contentful Assets finished.'))
